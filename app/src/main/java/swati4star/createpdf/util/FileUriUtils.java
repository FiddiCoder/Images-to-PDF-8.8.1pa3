...
public class FileUriUtils {
    ...
    protected boolean checkURIAuthority(Uri uri, String toCheckWith) {
        return toCheckWith.equals(uri.getAuthority());
    }

    protected boolean checkURI(Uri uri, String toCheckWith) {
     ...
     */
    protected boolean isDocumentUri(Context mContext, Uri uri) {
        boolean ret = false;
    ...
    protected String getURIForMediaDoc(ContentResolver mContentResolver, Uri uri) {
    ...
    protected String getURIForDownloadDoc(ContentResolver mContentResolver, Uri uri) {
        String documentId = DocumentsContract.getDocumentId(uri);
        // Build download uri.
    ...
    protected String getURIForExternalstorageDoc(Uri uri) {
        String documentId = DocumentsContract.getDocumentId(uri);
        String[] idArr = documentId.split(":");
        if (idArr.length == 2) {
            String type = idArr[0];
    ...
    protected String getUriForDocumentUri(ContentResolver mContentResolver, Uri uri) {
     ...
     */
    protected String getImageRealPath(ContentResolver contentResolver, Uri uri, String whereClause) {
        String ret = "";
